<?php
/**
 * Admin settings page — v3.3.
 *
 * Single-page, tab-free, mobile responsive. Sections:
 *   • Status & display (enable switch + target)
 *   • Preset picker (100 visual live-animated cards + search/filter)
 *   • Text & logo
 *   • Maintenance mode (enable, H/M/S, message)
 *   • Colors
 *   • Timing (hide moment: DOM-ready fast reveal or full page load)
 *   • Custom CSS
...
class ASPL_Settings {
	...
	public function sanitize( $input ) {
		...
		$clean['wait_images']        = ! empty( $input['wait_images'] ) ? 1 : 0;
		...
		// Hide moment: fast (DOM ready) or full page load.
		$hide_when = isset( $input['hide_when'] ) ? sanitize_key( $input['hide_when'] ) : 'dom_ready';
		$clean['hide_when'] = in_array( $hide_when, array( 'dom_ready', 'window_load' ), true ) ? $hide_when : 'dom_ready';
		...
		$clean['timeout']            = isset( $input['timeout'] ) ? max( 1, min( 600, absint( $input['timeout'] ) ) ) : 15;
		$clean['min_time']           = isset( $input['min_time'] ) ? max( 0, min( 10000, absint( $input['min_time'] ) ) ) : 350;
		$clean['fade_duration']      = isset( $input['fade_duration'] ) ? max( 100, min( 5000, absint( $input['fade_duration'] ) ) ) : 350;
	...
	/**
	 * Category label for a preset slug (used for the admin filter).
	 */
	private function preset_cat( $slug ) {
		$orig = array(
			'apple_star'     => 'apple',
			'pulse_classic'  => 'pulse',
			'freq_bars'      => 'eq',
			'sine_wave'      => 'wave',
			'ecg_heartbeat'  => 'ecg_line',
			'siri_orbit'     => 'orbit',
			'radar_sweep'    => 'radar',
			'breathing_core' => 'halo',
			'quantum_spin'   => 'atom',
	...
	/**
	 * Human Persian category names used in the filter dropdown.
	 */
	private function cat_labels() {
		return array(
			'apple'     => __( 'Apple Star', 'apple-star-loader' ),
			'pulse'     => __( 'پالس', 'apple-star-loader' ),
			'rings'     => __( 'حلقه‌ها', 'apple-star-loader' ),
			'halo'      => __( 'هاله', 'apple-star-loader' ),
			'bounce'    => __( 'جهش', 'apple-star-loader' ),
			'wave'      => __( 'موج', 'apple-star-loader' ),
			'eq'        => __( 'اکولایزر', 'apple-star-loader' ),
			'arc'       => __( 'کمان چرخان', 'apple-star-loader' ),
			'atom'      => __( 'اتم', 'apple-star-loader' ),
			'radar'     => __( 'رادار', 'apple-star-loader' ),
			'star'      => __( 'ستاره', 'apple-star-loader' ),
			'ecg_line'  => __( 'ضربان قلب', 'apple-star-loader' ),
			'orbit'     => __( 'مدار', 'apple-star-loader' ),
			'infinity'  => __( 'بی‌نهایت', 'apple-star-loader' ),
			'morph'     => __( 'مورف', 'apple-star-loader' ),
			'grid'      => __( 'ماتریس', 'apple-star-loader' ),
			'battery'   => __( 'باتری', 'apple-star-loader' ),
			'clock'     => __( 'ساعت', 'apple-star-loader' ),
			'aperture'  => __( 'دیافراگم', 'apple-star-loader' ),
			'pinwheel'  => __( 'پروانه', 'apple-star-loader' ),
			'drop'      => __( 'قطره', 'apple-star-loader' ),
			'tri'       => __( 'مثلث', 'apple-star-loader' ),
			'other'     => __( 'سایر', 'apple-star-loader' ),
		);
	}

	/**
	 * Live animated thumbnail for a preset card: the preset's own inline SVG
	 * with default placeholders applied. This keeps all 100 thumbnails in
	 * sync with the real loader (SMIL animations actually run).
	 */
	private function preset_thumb( $slug ) {
		$code = ASPL_Defaults::get_preset_code( $slug );
		if ( '' === trim( $code ) ) {
			return '<svg viewBox="0 0 80 40" xmlns="http://www.w3.org/2000/svg"><circle cx="40" cy="22" r="8" fill="currentColor"/></svg>';
		}
		if ( preg_match( '#<svg[\s\S]*?</svg>#i', $code, $m ) ) {
			$svg = $m[0];
		} else {
			return '<svg viewBox="0 0 80 40" xmlns="http://www.w3.org/2000/svg"><circle cx="40" cy="22" r="8" fill="currentColor"/></svg>';
		}
		// Apply the same substitutions the frontend does (default values).
		$svg = str_replace(
			array( '{{ACCENT}}', '{{TEXT_COLOR}}', '{{LOGO}}', '{{TEXT}}', '{{DIR}}', '{{MAINT}}' ),
			array( '#00c3ff', '#ffffff', '', '', 'dir="ltr"', '' ),
			$svg
		);
		return $svg;
	}
...
<div class="wrap aspl-admin">
	<div class="aspl-head">
		<div class="aspl-head-left">
			...
			<div>
				...
				<p><?php esc_html_e( '۱۰۰ مدل لودینگ اپل‌استایل برای وردپرس — شروع آنی انیمیشن، نمایش سریع صفحه، حالت بروزرسانی و پیش‌نمایش زنده.', 'apple-star-loader' ); ?></p>
			</div>
		</div>
	...
	<div class="aspl-grid">
		<form method="post" action="options.php" id="aspl-form" class="aspl-card-col">
			...
			<!-- Presets -->
			<section class="aspl-card">
				<h2>✨ <?php esc_html_e( 'انتخاب مدل لودینگ (۱۰۰ مدل)', 'apple-star-loader' ); ?></h2>
				<p class="aspl-section-hint"><?php esc_html_e( 'همه انیمیشن‌ها با SMIL SVG ساخته شده‌اند (مقاوم به بهینه‌سازها) و به محض ظاهر شدن صفحه شروع می‌شوند. روی هر کارت کلیک کنید تا مدل انتخاب شود؛ پیش‌نمایش زنده سمت راست بلافاصله به‌روز می‌شود.', 'apple-star-loader' ); ?></p>

				<div class="aspl-preset-tools">
					<input type="search" id="asplPresetSearch" class="aspl-text aspl-preset-search" placeholder="<?php esc_attr_e( 'جستجوی مدل… (مثلاً: رادار، موج، ستاره)', 'apple-star-loader' ); ?>">
					<select id="asplPresetCat" class="aspl-select aspl-preset-cat">
						<option value="all"><?php esc_html_e( 'همه دسته‌ها', 'apple-star-loader' ); ?></option>
						<?php foreach ( $cat_labels as $cat => $label ) : ?>
							<option value="<?php echo esc_attr( $cat ); ?>"><?php echo esc_html( $label ); ?></option>
						<?php endforeach; ?>
					</select>
					<span class="aspl-hint" style="margin:0;"><span id="asplPresetCount">100</span> <?php esc_html_e( 'مدل', 'apple-star-loader' ); ?></span>
				</div>

				<div class="aspl-preset-grid">
					<?php foreach ( $presets as $slug => $label ) : ?>
						<?php $cat = $this->preset_cat( $slug ); ?>
						<label class="aspl-preset-card <?php echo $opts['preset'] === $slug ? 'selected' : ''; ?>" data-slug="<?php echo esc_attr( $slug ); ?>" data-cat="<?php echo esc_attr( $cat ); ?>">
							<input type="radio" name="<?php echo esc_attr( self::OPTION ); ?>[preset]" value="<?php echo esc_attr( $slug ); ?>" <?php checked( $opts['preset'], $slug ); ?>>
							<span class="aspl-preset-thumb"><?php echo $this->preset_thumb( $slug ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
			...
			<section class="aspl-card">
				...
				<div class="aspl-row">
					<label class="aspl-lbl"><?php esc_html_e( 'رنگ پس‌زمینه لودینگ', 'apple-star-loader' ); ?></label>
					<input type="text" value="<?php echo esc_attr( $opts['bg_color'] ); ?>" name="<?php echo esc_attr( self::OPTION ); ?>[bg_color]" class="aspl-color" data-default-color="#0a0a0f">
				</div>
				<div class="aspl-row">
				...
				<div class="aspl-row">
					<label class="aspl-lbl"><?php esc_html_e( 'رنگ متن برند', 'apple-star-loader' ); ?></label>
					<input type="text" value="<?php echo esc_attr( $opts['text_color'] ); ?>" name="<?php echo esc_attr( self::OPTION ); ?>[text_color]" class="aspl-color" data-default-color="#ffffff">
				</div>
				<div class="aspl-row">
					<label class="aspl-lbl"><?php esc_html_e( 'رنگ تأکیدی (اکسنت)', 'apple-star-loader' ); ?></label>
					<input type="text" value="<?php echo esc_attr( $opts['accent_color'] ); ?>" name="<?php echo esc_attr( self::OPTION ); ?>[accent_color]" class="aspl-color" data-default-color="#00c3ff">
				</div>
				<div class="aspl-row">
			...
			<!-- Timing -->
			<section class="aspl-card">
				<h2>⏱️ <?php esc_html_e( 'زمان‌بندی (نمایش سریع)', 'apple-star-loader' ); ?></h2>

				<div class="aspl-row">
					<label class="aspl-lbl"><?php esc_html_e( 'کی لودینگ بسته شود؟', 'apple-star-loader' ); ?></label>
					<label style="display:block;margin-bottom:6px;">
						<input type="radio" name="<?php echo esc_attr( self::OPTION ); ?>[hide_when]" value="dom_ready" <?php checked( $opts['hide_when'], 'dom_ready' ); ?>>
						<strong><?php esc_html_e( 'به محض آماده‌شدن DOM (پیشنهادی، سریع)', 'apple-star-loader' ); ?></strong>
						<span class="aspl-hint" style="display:inline;margin:0;"><?php esc_html_e( '— صفحه پشتی تقریباً بلافاصله ظاهر می‌شود؛ انیمیشن از لحظه‌ی اول اجرا شده و لودینگ فقط چند صدم ثانیه نمایش داده می‌شود.', 'apple-star-loader' ); ?></span>
					</label>
					<label style="display:block;">
						<input type="radio" name="<?php echo esc_attr( self::OPTION ); ?>[hide_when]" value="window_load" <?php checked( $opts['hide_when'], 'window_load' ); ?>>
						<strong><?php esc_html_e( 'بعد از بارگذاری کامل صفحه (window load)', 'apple-star-loader' ); ?></strong>
						<span class="aspl-hint" style="display:inline;margin:0;"><?php esc_html_e( '— فقط برای صفحاتی که می‌خواهید همه‌ی تصاویر و فونت‌ها حتماً دانلود شوند (کندتر).', 'apple-star-loader' ); ?></span>
					</label>
				</div>
				<div class="aspl-row">
					<label class="aspl-switch">
						...
						<span class="slider"></span>
						<span class="lbl"><?php esc_html_e( 'در حالت «بارگذاری کامل» صبر تا لود آخرین عکس هم انجام شود', 'apple-star-loader' ); ?></span>
					</label>
				</div>
				<div class="aspl-row">
		...
		<aside class="aspl-side">
			...
			<section class="aspl-card">
				...
				<ul class="aspl-tips">
					<li><?php esc_html_e( 'انیمیشن SMIL به محض پارس شدن SVG شروع می‌شود — بدون تأخیر، مقاوم به بهینه‌سازها.', 'apple-star-loader' ); ?></li>
					<li><?php esc_html_e( 'حالت پیش‌فرض: لودینگ به محض آماده‌شدن DOM محو می‌شود تا سایت پشتی سریع دیده شود.', 'apple-star-loader' ); ?></li>
					<li><?php esc_html_e( 'برای جلوگیری از فلیکر، «حداقل زمان نمایش» را روی ۳۰۰ تا ۶۰۰ میلی‌ثانیه بگذارید.', 'apple-star-loader' ); ?></li>
...
.aspl-select{padding:8px 12px;border:1px solid var(--bd);border-radius:8px;background:#fff;min-width:220px;max-width:100%;}
...
/* Preset tools (search + category filter) */
.aspl-preset-tools{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:16px;}
.aspl-preset-search{max-width:340px;}
...
.aspl-preset-thumb svg{width:100%;height:56px;display:block;}
.aspl-preset-name{font-size:11.5px;font-weight:600;color:var(--tx);line-height:1.4;min-height:30px;display:flex;align-items:center;}
.aspl-preset-check{position:absolute;top:8px;inset-inline-end:8px;width:20px;height:20px;border-radius:50%;background:var(--p);color:#fff;font-size:12px;line-height:20px;text-align:center;opacity:0;transform:scale(.5);transition:.18s;}
.aspl-preset-card.selected .aspl-preset-check{opacity:1;transform:scale(1);}

/* Preview */
.aspl-framebox{background:#0a0a0f;border:1px solid #1f2937;border-radius:12px;padding:8px;}
...
CSS;
	...
	/* ============================================================
	   ...
	   for ANY selected preset. Supports logo, text, colors, timer,
	   and the new preset search/filter.
	   ============================================================ */
...
(function($){
	...
	function filterPresets(){
		var q = ($('#asplPresetSearch').val()||'').toLowerCase();
		var cat = $('#asplPresetCat').val()||'all';
		var visible = 0;
		$('.aspl-preset-card').each(function(){
			var ok = true;
			if(cat!=='all' && $(this).data('cat')!==cat) ok=false;
			if(ok && q){
				var hay = ($(this).find('.aspl-preset-name').text()+' '+( $(this).data('slug')||'')).toLowerCase();
				if(hay.indexOf(q)===-1) ok=false;
			}
			$(this).toggle(ok);
			if(ok) visible++;
		});
		$('#asplPresetCount').text(visible);
	}
	$('#asplPresetSearch').on('input',filterPresets);
	$('#asplPresetCat').on('change',filterPresets);

<?php
/**
 * Frontend — v3.1.0 (EARLY-PAINT architecture).
 *
 * Two core guarantees:
 *   1. The loader is the FIRST content the browser paints:
 *        - Critical CSS is injected in <head> at priority -99999 (before theme CSS).
 *        - Loader HTML is injected at the top of <body> via an output-buffer
 *          that inserts immediately after the opening <body> tag, so it works
 *          even on themes that forget to call wp_body_open().
 *        - <html> background is forced to the loader bg from the very first
 *          style rule, preventing any white flash.
 *        - Control JS (fade / image-wait / countdown) is placed at the end of
 *          <body> so it never blocks initial paint of the SVG.
 *   2. Animations are 100% SVG SMIL (<animate>/<animateTransform> inside the
 *      inline SVG). SMIL starts the moment the browser parses the <svg> tag —
 *      no JS, no CSS @keyframes, nothing optimizer plugins can strip.
 *   3. Brand text is STATIC (no per-letter spans, no text animation) — matches
...
class ASPL_Frontend {
	...
	public function should_render() {
		...
		if ( ! empty( $o['hide_for_logged_in'] ) ) {
			// "hide for logged in" should not hide for admins who want to preview — respect the setting strictly.
			if ( is_user_logged_in() ) return false;
		}
		if ( empty( $o['show_on_mobile'] ) && wp_is_mobile() ) return false;
		if ( isset( $o['maintenance_mode'] ) && $o['maintenance_mode'] ) return true;
		$t = isset( $o['target'] ) ? $o['target'] : 'front_page';
		if ( 'front_page' === $t && ! is_front_page() ) return false;
		if ( 'all_pages'  === $t ) return true;
		if ( 'home_posts' === $t && ! ( is_front_page() || is_home() ) ) return false;
		if ( 'posts_only' === $t && ! is_singular( 'post' ) ) return false;
		if ( 'pages_only' === $t && ! is_page() ) return false;
		if ( 'woocommerce' === $t ) {
	...
	public function print_head_css() {
		...
		?>
<!-- Apple Star Page Loader v3.1.0 (early-paint critical CSS) -->
...
<noscript><style>#asp-loader-root{display:none!important;}html,body{overflow:auto!important;background:<?php echo esc_attr( $solid_bg ); ?>;}</style></noscript>
	...
	private function build_body_html() {
		...
		// Immediately apply scroll-lock so the page can't scroll behind the loader.
		// This runs SYNCHRONOUSLY the moment the loader tag is parsed, before any
		// other body content paints — no flicker.
